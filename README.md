...
1. **主電腦**（筆電／接投影）：建立雲端比賽，負責選手、對戰表、鎖定輪次、公佈排名。
2. **手機／iPad 裁判**：同一網站 → **加入比賽** → 揀 **計分板** → 填比賽 ID + 主持碼。畫面只有大掣：揀場 → 撳邊個贏 → 撳 Extreme／Over／Burst／Spin。分數即時上主電腦。右上角 **拍片** 會開手機鏡頭；影片只存你部機（相簿／檔案），**唔會**上網站或雲端。錄完會返計分板。
3. **第二部投影**：加入時揀只讀，唔填主持碼。
4. 計分板亦可本機撳「開啟計分板」試用（唔使雲端）。直達連結：`https://takjai18.github.io/baoluo-cup-v2/?pad=1`
...
2. **多裝置即時同步**：設定 Firebase 後，用**比賽 ID** 連同一場（見下節）。
